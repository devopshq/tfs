...
class TFSAPI:
    def __init__(
        ...
        if auth_type is HTTPBasicAuth:
            if (user is None or password is None) and pat is None:
                raise ValueError(
                    "User name and password or personal access token must be specified!"
                )
        self.rest_client = TFSHTTPClient(
            server_url,
            project=project,
            user=user,
            password=password,
    ...
    def get_query_details(self, uri, underProject=None, payload=None):
    ...
    def __get_workitems(self, work_items_ids, fields=None, expand="all"):
        ...
        workitems = self.get_tfs_resource(
            "wit/workitems?ids={ids}{fields}{expand}&api-version=1.0".format(
                ids=ids_string, fields=fields_string, expand=expand
            ),
            underProject=False,
        )
        return workitems
    ...
    def get_workitems(self, work_items_ids, fields=None, batch_size=50, expand="all"):
        ...
        for work_items_batch in batch(list(work_items_ids), batch_size):
            work_items_batch_info = self.__get_workitems(
                work_items_batch, fields=fields, expand=expand
            )
    ...
    def get_changesets(self, from_=None, to_=None, item_path=None, top=10000):
        ...
        changesets = self.get_tfs_resource(
            "tfvc/changesets", underProject=False, payload=payload
        )
        return changesets
    ...
    def teams(self, projectId):
        ...
        """
        return self.get_tfs_resource(
            "projects/{}/teams".format(projectId), underProject=False
        )
    ...
    def definitions(self, nameFilter=None):
        ...
        if nameFilter:
            ...
            payload = None
        return self.get_tfs_resource(
            "build/definitions", underProject=True, payload=payload
        )
    ...
    def results(self, runId, top=None):
        ...
        payload = {"$top": top}
        return self.get_tfs_resource(
            "test/runs/{}/results".format(runId), underProject=True, payload=payload
        )
    ...
    def update_workitem(self, work_item_id, update_data, params=None):
        raw = self.rest_client.send_patch(
            "wit/workitems/{id}?api-version=6.0-preview.3".format(id=work_item_id),
            data=update_data,
    ...
    def run_query_tfs_resource(self, path):
        """Get query definition by path
        and get Wiql of this query results in the self.result
        """
        return self.get_query_details(f"wit/queries/{path}?$depth={2}",underProject=True)
    ...
    def run_wiql(self, query, params=None):
        ...
        wiql = self.rest_client.send_post(
            "wit/wiql", data=data, project=True, payload=params
        )
    ...
    def __create_workitem(
        ...
        headers = {"Content-Type": "application/json-patch+json"}
        raw = self.rest_client.send_post(
            uri=uri, data=data, headers=headers, project=True, payload=params
        )
        return raw
    ...
    def create_workitem(
        ...
        body = (
            [
                dict(op="add", path="/fields/{}".format(name), value=value)
        ...
        if relations_raw:
            body.extend(
                [
                    dict(op="add", path="/relations/-", value=relation)
                    for relation in relations_raw
                ]
            )

        raw = self.__create_workitem(
            type_,
            body,
            validate_only,
            bypass_rules,
    ...
    def copy_workitem(
        self,
        workitem,
        with_links_and_attachments=False,
        from_another_project=False,
        target_type=None,
        target_area=None,
        ...
        if from_another_project:
            ...
            fields["System.AreaPath"] = (
                target_area
                if target_area
                else self.__adjusted_area_iteration(workitem["AreaPath"])
            )
            fields["System.IterationPath"] = (
                target_iteration
                if target_iteration
                else self.__adjusted_area_iteration(workitem["IterationPath"])
            )
...
class TFSHTTPClient:
    ...
    def send_get(self, uri, payload=None, project=False, json=True):
        return self.__send_request(
            "GET", uri, None, payload=payload, underProject=project, json=json
        )

    def send_post(self, uri, data, headers=None, payload=None, project=False):
        return self.__send_request(
            "POST", uri, data, headers, payload=payload, underProject=project
        )

    def send_put(self, uri, data, headers=None, payload=None, project=False):
        return self.__send_request(
            "PUT", uri, data, headers, payload=payload, underProject=project
        )

    def send_patch(self, uri, data, headers, payload=None, project=False):
        return self.__send_request(
            "PATCH", uri, data, headers, payload=payload, underProject=project
        )
    def send_delete(self, uri, headers, payload=None, project=False):
        return self.__send_request(
            "DELETE", uri, headers, payload=payload, underProject=project, json=False
        )
        ...
        if json:
            try:
                ...
                if response.status_code not in (200, 201, 202):
                    raise TFSClientError(
                        "TFS API returned HTTP %s (%s)"
                        % (
                            response.status_code,
                            result["error"] if "error" in result else response.reason,
                        )
                    )
                return result
            except ValueError:
